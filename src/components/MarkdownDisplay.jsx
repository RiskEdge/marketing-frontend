...
const MarkdownDisplay = ({ markdownText }) => {
	...
	const components = {
		...
		h2: ({ node, ...props }) => (
			<h2 className='text-base md:text-xl font-semibold my-3' {...props} />
		),
		h3: ({ node, ...props }) => (
			<h3 className='text-base md:text-xl font-medium my-2' {...props} />
		),
		h4: ({ node, ...props }) => (
			<h3 className='text-lg md:text-2xl font-medium my-2' {...props} />
		),

		// Customize paragraphs
		p: ({ node, ...props }) => (
			<p className='text-sm md:text-base my-2 leading-6 md:leading-7' {...props} />
		),
		...
		li: ({ node, ...props }) => (
			<li className='text-sm leading-6 md:leading-7 md:text-base my-1 text-wrap' {...props} />
		),
		...
		pre: ({ node, children, ...props }) => {
			if (!children || children.length === 0) return <span></span>;
			return (
				<pre className='bg-gray-800 p-4 rounded my-2 text-wrap' {...props}>
					{children}
				</pre>
			);
		},
